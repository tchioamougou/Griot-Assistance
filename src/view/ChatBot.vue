<template>
    ...
    <div style="box-shadow: 0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05);" v-if="display"
        ...
        <!-- Chat Container -->
        <div class="pr-4 h-[474px] overflow-auto" style="min-width: 100%; display: block;" id="messageBody">
            <template v-for="(ch, i) in chatMessages" :key="i">
                <!-- Chat Message AI -->
                <div class="flex gap-3 my-4 text-gray-600 text-sm flex-1" v-if="ch.user === 'boot'"><span
                ...
                <!--  User Chat Message -->
                <div class="flex gap-3 my-4 text-gray-600 text-sm flex-1" v-else><span
                        class="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8">
                        <div class="rounded-full bg-gray-100 border p-1"><svg stroke="none" fill="black"
                    ...
                    <p class="leading-relaxed"><span class="block font-bold text-gray-700">You </span>{{ ch.message }}
                    </p>
                </div>
            </template>
        </div>
        ...
        <div class="flex items-center pt-0 mt-2">
            <form @submit.prevent="streamContent" class="flex items-center justify-center w-full space-x-2">
...
import { hasQuestion } from '../gemini/service';
...
const sendQuestion = async () => {
    ...
    const res = await hasQuestion(search.value);
    chatMessages.value.push({ user: "boot", message: res });
    search.value = '';
    isLoanding.value = false;
    localStorage.setItem('chats', JSON.stringify(chatMessages.value));
}
const streamContent = async () => {
    isLoanding.value = true;
    chatMessages.value.push({ user: "you", message: search.value });
    const result = await model.generateContentStream(search.value);
    chatMessages.value.push({ user: "boot", message: "" });
    let text = ""
    for await (const chunk of result.stream) {
        const chunkText = chunk.text();
        console.log(chunkText);
        text += chunkText;
        chatMessages.value[chatMessages.value.length - 1].message = convertirTexteEnHTML(text);
        scrollDown()
    }
    isLoanding.value = false;
    search.value = '';
...
onBeforeMount(() => {
    const chatsResult = localStorage.getItem('chats');
    const chats = chatsResult ? JSON.parse(chatsResult) : [{ user: 'boot', message: "How can I help you?" }];
