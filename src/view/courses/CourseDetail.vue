...
const exportAsPdf = async () => {
    const modules = props.course.modules.map((e, i) => {
        return [
            ...
            { text: `${e.description}` },
            { text: `${t('Objectives')}`, style: "objectifs", margin: [0, 10] },
            {
            ...
            e.lessons.map((le, i) => {
                return [
                    { text: `${t('Lesson')}: ${i + 1}: ${le.name}`, style: "module", margin: [0, 10] },
                    { text: `${le.description}` },
                    { text: `${t('Objectives')}`, style: "objectifs", margin: [0, 10] },
                    {
    ...
    const docDefinition = {
        ...
        content:
            [
                { text: `${t('Course Title')} ${props.course.title}`, style: 'header', margin: [0, 10] },
                { text: `${props.course.description}`, margin: [0, 5] },
                { text: `${t('Time')} ${props.course.time}`, margin: [0, 5] }
            ].concat(modules),
        styles: {
            header: {
                fontSize: 22,
